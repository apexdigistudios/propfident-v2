...
import { AnchoredToastProvider, anchoredToastManager } from "@/components/ui/toast";
import { ArrowRight, HelpCircle, Loader2, Sparkles, Users } from "lucide-react";

export default function MillionaireRoadmapTeaserPage() {
  const [email, setEmail] = useState("");
  const [loading, setLoading] = useState(false);
  const bottomSubmitRef = useRef<HTMLButtonElement>(null);
  ...
  const handleWaitlistSubmit = async (
    ...
    if (!emailToSubmit || loading) return;
  ...
  const handleBottomSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    handleWaitlistSubmit(email, bottomSubmitRef.current);
  };
  ...
  return (
    <AnchoredToastProvider>
      <main className="min-h-screen bg-background text-foreground selection:bg-primary/20 selection:text-primary pt-16 sm:pt-0">
        ...
        <RoadmapTeaserHero
          onSubscribe={(email, buttonEl) => handleWaitlistSubmit(email, buttonEl)}
          loading={loading}
        />
